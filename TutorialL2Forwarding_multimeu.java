...
public class TutorialL2Forwarding implements IListenDataPacket {
    ...
    public PacketResult receiveDataPacket(RawPacket inPkt) {
        ...
        // Store srcMAC and incoming_connector in the mapping dictionary of each DIFFERENT SWITCH
        if(!mac_to_port_per_switch.containsKey(incoming_node)) {
        	mac_to_port_per_switch.put(incoming_node, new HashMap<Long, NodeConnector>());
        }
        mac_to_port_per_switch.get(incoming_node).put(srcMAC_val, incoming_connector);
        
        // Do I know the destination MAC? Lookup dstMAC in dictionary
        // If found, generate match and action for the flow, and program a
        // flow rule in switch
        boolean isDstMACKnown = mac_to_port_per_switch.get(incoming_node).containsKey(dstMAC_val);
        
        if (isDstMACKnown) {
        	NodeConnector dst_connector = mac_to_port_per_switch.get(incoming_node).get(dstMAC_val);
            ...
            // Modify the flow on the network node
             ...
             if (!status.isSuccess()) {
            	 ...
            	 try {
     				inPkt.setOutgoingNodeConnector(dst_connector);
